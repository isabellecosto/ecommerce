...
import { IconShoppingBag, IconShoppingCartPlus } from "@tabler/icons-react"
...
export default function ProdutoItem(props: ProdutoItemProps) {
    const { produto } = props
    return (
    <Link href={`/produto/${produto.id}`} className='flex flex-col bg-violet-dark border border-white/10 rounded/xl relative max-w-[350px]' >
        ...
        <button className="
            flex justify-center h-8 gap-2 items-center h-8
